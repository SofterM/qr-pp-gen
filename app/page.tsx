    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `จ่ายเงินผ่าน PromptPay ${amount ? `จำนวน ${amount} บาท` : ""}`,
          url: shareUrl,
        })
      } catch (error) {
        copyShareUrl()
        ...
        <Card className="w-full max-w-md shadow-2xl">
          <CardHeader className="text-center pb-4">
            <div className="flex items-center justify-center gap-2 mb-2">
              ...
              <CardTitle className="text-xl">PromptPay</CardTitle>
            </div>
            <CardDescription>สแกนเพื่อจ่ายเงิน</CardDescription>
          </CardHeader>

          <CardContent className="space-y-6">
            <div className="bg-white dark:bg-gray-50 p-6 rounded-xl shadow-inner">
              <img
            ...
            <div className="space-y-3">
              ...
              {amount && (
                <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                  ...
                  <span className="font-bold text-lg">฿{Number.parseFloat(amount).toLocaleString()}</span>
                </div>
              )}
            </div>

            <div className="text-center space-y-3">
              <p className="text-xs text-muted-foreground">สแกน QR Code ด้วยแอปธนาคารของคุณ</p>
